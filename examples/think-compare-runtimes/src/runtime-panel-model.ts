...
export type SegmentStatus = "running" | "passed" | "failed" | "neutral" | "lease";
...
export function buildRuntimePanelModel(
  ...
  const lanes = runtime === "workspace" ? workspaceLanes(runtimeEvents) : sandboxLanes(runtimeEvents);
...
function workspaceLanes(events: RunEvent[]): TimelineLane[] {
  return [
    { id: "files", label: "Files", tone: "vfs", segments: [], markers: fileMarkers(events, "vfs") },
    {
      id: "dynamic-worker",
      label: "Dynamic Worker",
      tone: "dynamic-worker",
      segments: execSegments(events, "dynamic-worker"),
      markers: [],
    },
    {
      ...
      label: "Sandbox",
      tone: "container",
      segments: containerSegments(events, "workspace-sandbox"),
      markers: [],
    },
  ];
}
...
function sandboxLanes(events: RunEvent[]): TimelineLane[] {
  ...
  return [
    { id: "files", label: "Files", tone: "vfs", segments: [], markers: [] },
    { id: "dynamic-worker", label: "Dynamic Worker", tone: "dynamic-worker", segments: [], markers: [] },
    {
      id: "raw-sandbox",
      label: "Sandbox",
      tone: "container",
      segments: [...bootSegment, ...containerSegments(events, "raw-sandbox")],
...
function containerSegments(events: RunEvent[], target: ExecutionTarget): TimelineSegment[] {
  return [...containerLeaseSegments(events, target), ...execSegments(events, target)].sort(
    (left, right) => left.startMs - right.startMs || segmentOrder(left.status) - segmentOrder(right.status),
  );
}
...
function containerLeaseSegments(events: RunEvent[], target: ExecutionTarget): TimelineSegment[] {
  ...
  for (const event of events) {
    const fact = factForEvent(event);
    if (fact.executionTarget !== target) continue;
...
function workItems(events: RunEvent[]): AgentWorkItem[] {
  ...
  const flushReads = () => {
    ...
    const suffix = readAggregate.paths.length > 3 ? ` · +${readAggregate.paths.length - 3} more` : "";
    items.push({
  ...
  const appendFinalMessage = (event: RunEvent) => {
    ...
    if (existing) {
      ...
      existing.text = streamed === final || streamed.includes(final) ? streamed : [streamed, final].filter(Boolean).join("\n\n");
  ...
  for (const event of events) {
    ...
    if (event.kind === "agent_message" && event.title === "Think turn started") continue;
    if (event.kind === "agent_message" && event.title !== "Think turn complete") {
      ...
      items.push({
        ...
        label: event.title,
        text: event.detail,
        tone: event.title.includes("Retrying") ? "error" : "neutral",
        presentation: "compact",
      });
      continue;
    }
    ...
    if (!isAgentToolLifecycleEvent(event)) continue;
...
function isAgentToolLifecycleEvent(event: RunEvent): boolean {
  return event.kind === "agent_tool_call" || event.kind === "agent_tool_result" || event.kind === "agent_tool_error";
}
...
function execItem(
  ...
  return {
    id: fact.event.id,
    kind: "exec",
    label: fact.failed ? "Command failed" : target === "dynamic-worker" ? "Ran Dynamic Worker" : "Ran command",
    text: `${command}${target ? ` · ${executionTargetLabel(target)}` : ""}${typeof fact.exitCode === "number" ? ` · exit ${fact.exitCode}` : ""}`,
...
function fileMutationItem(
  ...
  return {
    ...
    text: `${displayPath} · ${fact.failed ? "failed" : "applied"}`,
    tone: fact.failed ? "error" : "success",
    presentation: "compact",
  };
}
    ...
    for (const fact of calls) {
      if (fact.tool === "exec" && fact.command) {
        items.push({
          id: fact.event.id,
          kind: "exec",
          label: fact.executionTarget === "dynamic-worker" ? "Dynamic Worker requested" : "Command requested",
          text: fact.command,
          tone: "neutral",
          presentation: "terminal",
...
function executionTargetLabel(target: ExecutionTarget): string {
  if (target === "dynamic-worker") return "dynamic worker";
  if (target === "workspace-sandbox") return "Workspace Sandbox";
  return "raw Sandbox";
}
...
function transcriptItems(events: RunEvent[]): TranscriptItem[] {
  ...
  if (failure) return [{ id: failure.id, text: failure.detail, tone: "error" }];
  if (final) return [{ id: final.id, text: final.detail, tone: "success" }];
  return [];
}
...
function runtimeClock(events: RunEvent[], telemetry: RuntimeDashboardModel): RuntimeClock {
  ...
  return { startMs, endMs, durationLabel: telemetry.elapsedLabel };
}
...
function summaryItems(runtime: RuntimeId, telemetry: RuntimeDashboardModel): RuntimeSummaryItem[] {
  if (runtime === "workspace") {
    return [
      ...
      { label: "Dynamic worker", value: String(telemetry.dynamicWorkerExecs) },
      { label: "Sandbox commands", value: String(telemetry.sandboxExecs) },
    ];
  }
