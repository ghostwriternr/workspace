    ...
    >
      {items.map((item) => (
        <div className="border-[#DED8CD] border-r px-3 py-2 last:border-r-0" key={item.label}>
          ...
          <dd className="mt-1 font-mono text-lg tracking-[-0.05em] text-[#111111]">
            {item.value}
          </dd>
        </div>
      ))}
...
function TimelineLegend() {
  return (
    <div className="mb-3 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[0.62rem] tracking-[0.12em] text-[#8F8A81] uppercase">
      <LegendSwatch className="bg-[#1D4ED8]" label="files" />
      <LegendSwatch className="bg-[#7C3AED]" label="dynamic worker" />
      <LegendSwatch className="bg-[#F4C98F] opacity-55" label="sandbox assigned" />
...
function AgentWorkRow({ item }: { item: AgentWorkItem }) {
  if (item.presentation === "terminal") return <TerminalWorkRow item={item} />;
  if (item.presentation === "compact") return <CompactWorkRow item={item} />;
...
function CompactWorkRow({ item }: { item: AgentWorkItem }) {
  return (
    <li className="flex items-baseline gap-3 py-1 text-sm leading-5">
      <span className={`font-mono text-[0.65rem] tracking-[0.16em] uppercase ${compactToneClass(item)}`}>
        {item.label}
      </span>
  ...
  return (
    <li className="py-1.5 text-sm leading-5">
      <div className="flex flex-wrap items-baseline gap-2 font-mono text-[0.62rem] tracking-[0.14em] uppercase">
        ...
        {item.executionTarget ? <span className="text-[#8F8A81]">{executionTargetCopy(item.executionTarget)}</span> : null}
...
function workStreamWatchKey(items: AgentWorkItem[]): string {
  const textLength = items.reduce(
    (total, item) => total + item.text.length + (item.stdout?.length ?? 0) + (item.stderr?.length ?? 0),
    0,
  );
...
function capacityHint(error: string | null): string | null {
  ...
  return error.includes("Capacity temporarily exceeded") ? "Upstream model capacity; retry later." : null;
}
