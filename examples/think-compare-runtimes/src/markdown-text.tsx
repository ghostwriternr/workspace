...
function Heading2(props: ComponentProps<"h2"> & { node?: unknown }) {
  ...
  return <h2 className="mt-3 text-base font-semibold tracking-[-0.02em] text-[#111111]" {...rest} />;
}
...
function OrderedList(props: ComponentProps<"ol"> & { node?: unknown }) {
  const { node: _node, ...rest } = props;
  return <ol className="mt-2 ml-5 grid list-decimal gap-1 text-sm leading-6 text-[#24211D]" {...rest} />;
}

function UnorderedList(props: ComponentProps<"ul"> & { node?: unknown }) {
  const { node: _node, ...rest } = props;
  return <ul className="mt-2 ml-5 grid list-disc gap-1 text-sm leading-6 text-[#24211D]" {...rest} />;
}
...
function getHighlighter(): Promise<HighlighterCore> {
  ...
  ]).then(([core, engine, typescript, javascript, tsx, shellscript, json, jsonc, markdown, theme]) =>
