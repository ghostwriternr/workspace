export function codingAgentPrompt(workspaceName: string): string {
  return [
    ...
    "Workspace stores durable repository files. Dynamic Workers are the execution substrate behind Workspace Worker tool calls.",
    "The browser imports public GitHub repositories before the edit loop begins.",
    "Use runWorkspaceWorker for inspection and edits through env.WORKSPACE. Delegated code receives readFile, writeFile, list, and stat.",
    "Workspace file methods return plain objects: { status: 'ok', value } or { status: 'error', error }.",
    "Workspace Worker code must default-export an async function that accepts env.",
    "Check Workspace method status before using values, and return or summarize errors clearly.",
